...
function fieldErrorCheck(field, modal){
	if($("#"+field+"_controls"+modal+" .help-inline").length){
		...
		$("#"+field+"_Help-Inline"+modal+" .alert alert-danger").removeClass("label label-warning label-success").addClass("label label-danger");
	}else{
		//create it
		$("#"+field+"_controls"+modal).append('<span id = "'+field+'Help-Inline'+modal+'" class = "help-inline"></span>');
		$("#"+field+"_Help-Inline"+modal).append('<span class = "alert alert-danger"></span>');
	}
}
...
function errorNote(field, func, text, modal){
	if(text !== ""){
		$("#"+field+"_Help-Inline"+modal+" .alert alert-danger").removeClass("label label-warning label-success").addClass("label label-danger");
		$("#"+field+"_Help-Inline"+modal+" .alert alert-danger").text(text);
	}
...
function validateUser(evt){
	...
	if(test === true){
		$("#username_Help-Inline"+modal+" .alert alert-danger").removeClass("label label-danger label-success").addClass("label label-warning");
		$("#username_Help-Inline"+modal+" .alert alert-danger").text(_s_checking);
		myTimeout = window.setTimeout(function() {
			$.ajax({url:"/account/profile/username_exists/"+value}).done(function(check){
				if(checkUser($("#sign_up_username"+modal).val(), modal) === true){
					if(check == 1){
						$("#username_Help-Inline"+modal+" .alert alert-danger").removeClass("label label-warning label-success").addClass("label label-danger");
						$("#username_Help-Inline"+modal+" .alert alert-danger").text(_u_alreadyExists);
					}else{
						$("#username_Help-Inline"+modal+" .alert alert-danger").removeClass("label label-danger label-warning").addClass("label label-success");
						$("#username_Help-Inline"+modal+" .alert alert-danger").text(_u_avail);
					}
				}
			});}, 300);
	}
...
function checkUser(value, modal){
	...
	if(value === ""){
		$("#username_Help-Inline"+modal+" .alert alert-danger").removeClass("label label-warning label-success").addClass("label label-danger");
		$("#username_Help-Inline"+modal+" .alert alert-danger").text(_u_noUsername);
		return false;
	}else if(value.length < 2){
		$("#username_Help-Inline"+modal+" .alert alert-danger").removeClass("label label-warning label-success").addClass("label label-danger");
		$("#username_Help-Inline"+modal+" .alert alert-danger").text(_u_tooShort);
		return false;
	}else if(value.length > 24){
		$("#username_Help-Inline"+modal+" .alert alert-danger").removeClass("label label-warning label-success").addClass("label label-danger");
		$("#username_Help-Inline"+modal+" .alert alert-danger").text(_u_tooLong);
		return false;
	}else if(!re.test(value)){
		$("#username_Help-Inline"+modal+" .alert alert-danger").removeClass("label label-warning label-success").addClass("label label-danger");
		$("#username_Help-Inline"+modal+" .alert alert-danger").text(_u_inVaildChars);
		return false;
	}else{
		return true;
	}
...
function validateEmail(evt)
{
	...
	if(!re.test(value)){
		$("#email_Help-Inline"+modal+" .alert alert-danger").removeClass("label label-warning label-success").addClass("label label-danger");
		$("#email_Help-Inline"+modal+" .alert alert-danger").text(_e_invaild);
	}else{
		$("#email_Help-Inline"+modal+" .alert alert-danger").removeClass("label label-danger label-warning").addClass("label label-success");
		$("#email_Help-Inline"+modal+" .alert alert-danger").text(_e_vaild);
	}
}
...
{
	var modal = $(this).attr("id").indexOf("modal") > 0 ? "_modal" : "";
	var phife = $("#password_Help-Inline"+modal+" .alert alert-danger");
	var cphife = $("#confirm_password_Help-Inline"+modal+" .alert alert-danger");
